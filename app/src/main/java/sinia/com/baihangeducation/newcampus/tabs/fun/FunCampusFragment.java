...
import sinia.com.baihangeducation.newcampus.tabs.fun.adapter.AddHeaderAdapter;
...
public class FunCampusFragment extends FragmentLazy implements IGetFunView, IIsShowInput, View.OnClickListener {
    ...
    private FunCampusPresenter mFunCampusPresenter;
    ...
    public void initView() {
        mSuperRecyclerView = mRoot.findViewById(R.id.recyclerView);
        mRefreshLayout = mRoot.findViewById(R.id.refreshLayout);
        mLayout = mRoot.findViewById(R.id.newcampany);
        mInput = mRoot.findViewById(R.id.newcampany_input);
        mErrorImageView = mRoot.findViewById(R.id.img_state);
        mErrorTextView = mRoot.findViewById(R.id.tv_state);

        mSuperRecyclerView.setLayoutManager(new GridLayoutManager(getContext(), 1));
        mNewCampusFunAdapter = new NewCampusFunAdapter(getContext(), mList, mLayout, this, true);
        mSuperRecyclerView.setAdapter(mNewCampusFunAdapter);


        mFunCampusPresenter = new FunCampusPresenter(new FunCampusModel(getActivity()), FunCampusFragment.this);
        hotFunListModel = new HotFunListModel(getActivity());
    ...
    private void addHeaderView() {
        View header = LayoutInflater.from(getContext()).inflate(R.layout.activity_recyclerview_head, null);
        ...
        intent = new Intent(getContext(), HotCommentPageActivity.class);
    }
    ...
    public void getData() {
        ...
        hotFunListModel.getHotFunList(1, 1, new HotFunDataListener() {
            ...
            public void getHotFunDataSuccess(HotFunListBean hotFunListBean) {
                FunCampusFragment.this.hotFunListBean = hotFunListBean;
                if (hotFunListBean != null && hotFunListBean.list != null && hotFunListBean.list.size() >= 1) {
                    ...
                    Glide.with(getActivity()).load(hotFunListBean.list.get(0).topic_logo).error(R.drawable.logo).into(imageViewOne);
                    Glide.with(getActivity()).load(hotFunListBean.list.get(1).topic_logo).error(R.drawable.logo).into(imageViewTwo);
                    Glide.with(getActivity()).load(hotFunListBean.list.get(2).topic_logo).error(R.drawable.logo).into(imageViewThree);
                    Glide.with(getActivity()).load(hotFunListBean.list.get(3).topic_logo).error(R.drawable.logo).into(imageViewFour);
    ...
    private void showErrorState(boolean isError) {
        if (isError) {
            ...
            Glide.with(getContext()).load(R.mipmap.ic_launcher).into(mErrorImageView);
    ...
    protected void hideSoftInput(EditText view) {
        InputMethodManager mInputMethodManager = (InputMethodManager) getContext().getSystemService(getContext().INPUT_METHOD_SERVICE);
        mInputMethodManager
